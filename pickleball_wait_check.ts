...
async function main() {
  ...
  console.log('Waiting 5 seconds for UI to update...');
  await page.waitForTimeout(5000);
  
  await page.screenshot({ path: 'pickleball_after_wait.png', fullPage: true });
  console.log('✓ Saved pickleball_after_wait.png');
  
  // Check Next button state
  const buttonState = await page.evaluate(() => {
    const nextBtn = document.querySelector('button');
    const allButtons = Array.from(document.querySelectorAll('button'));
    const nextButtons = allButtons.filter(b => b.textContent?.includes('NEXT'));
    
    return {
      nextButtonsFound: nextButtons.length,
      states: nextButtons.map(b => ({
