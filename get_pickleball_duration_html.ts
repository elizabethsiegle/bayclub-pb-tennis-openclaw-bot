...
async function main() {
  ...
  // Get HTML of the duration area
  const html = await page.evaluate(() => {
    // Find app-button-select (duration selector component)
    const buttonSelect = document.querySelector('app-button-select');
    return {
      exists: !!buttonSelect,
      html: buttonSelect?.outerHTML.substring(0, 2000) || 'NOT FOUND',
      innerText: buttonSelect?.innerText || '',
    };
  });
  
  console.log('\nDuration selector HTML:');
